...
def max_pool(x):
    return tf.nn.max_pool(x, [1, 5, 5, 1], [1, 1, 1, 1], padding = "SAME")

def conv_bn_relu(input_, in_channel, out_channel, name="conv", reuse = False):
    with tf.variable_scope(name, reuse = reuse):
        w = tf.get_variable("W", shape = [5, 5, in_channel, out_channel], initializer = tf.contrib.layers.xavier_initializer())
...
def conv_tran_bn_relu(input_, output_shape, name="conv_t", reuse = False):
    with tf.variable_scope(name, reuse = reuse):
        w = tf.get_variable("W", shape = [4, 4, output_shape[-1], input_.shape[-1]], initializer = tf.contrib.layers.xavier_initializer())
        b = tf.get_variable("B", shape = [1], initializer = tf.zeros_initializer())
...
def dense_sigmoid(input_, neurons, name="dense", reuse = False):
    with tf.variable_scope(name, reuse = reuse):
        w = tf.get_variable("W", shape = [input_.shape[1], neurons], initializer = tf.contrib.layers.xavier_initializer())
        b = tf.get_variable("B", shape = [1], initializer = tf.zeros_initializer())
